...
void blink(const struct gpio *led, const struct gpio *pin, uint32_t period_ms, uint32_t ontime_ms, uint32_t id)
{
	...
	if (!device_is_ready(spec_trig->port)) {
	...
	while (1) {
		...
		gpio_pin_set(spec_trig->port, spec_trig->pin, 1);  // 1=ON
		k_msleep(ontime_ms);
		
		/* OFF phase */
		gpio_pin_set(spec->port, spec->pin, 0);
		gpio_pin_set(spec_trig->port, spec_trig->pin, 0);  // 0=OFF
		...
		int64_t remaining = period_ms - elapsed;
		if (remaining > 0) {
			k_msleep(remaining);
		}
	}
